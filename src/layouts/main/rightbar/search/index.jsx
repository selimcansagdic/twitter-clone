    ...
    <div ref={ref} className="min-h-[32px] h-[53px] mb-3 flex items-center sticky top-0 bg-black z-[10]  ">
      <label className="h-[43px] rounded-full bg-[#202327] w-full relative group border border-transparent focus-within:bg-black focus-within:border-[#1d9bf0] ">
        <div className="w-[56px] h-full flex items-center justify-center absolute top-0 left-0 pointer-events-none  ">
          <svg viewBox="0 0 24 24" height={18.75} className="h-4 min-w-[32px] text-primary group-focus-within:text-[#1d9bf0] ">
            <path
              fill="currentColor"
        ...
        <input
          type="text"
          placeholder="Search"
          className="w-full h-full bg-transparent placeholder-primary rounded-full outline-none pl-[56px] text-[15px]  "
          value={query}
      ...
      {focus && (
        <div className="absolute w-[350px] top-full -translate-y-1 -left-px bg-black shadow-box max-h-[calc(80vh-53px)] rounded-lg text-center min-h-[100px]">
          <p className="p-3 pt-5  text-primary leading-5">Try searching for people, lists or keywords</p>
        </div>
      )}
    </div>
  );
